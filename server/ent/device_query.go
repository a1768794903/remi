...
func (_q *DeviceQuery) QueryConversations() *ConversationQuery {
...
func (_q *DeviceQuery) WithConversations(opts ...func(*ConversationQuery)) *DeviceQuery {
	query := (&ConversationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := _q.withConversations; query != nil {
		if err := _q.loadConversations(ctx, query, nodes,
			func(n *Device) { n.Edges.Conversations = []*Conversation{} },
			func(n *Device, e *Conversation) { n.Edges.Conversations = append(n.Edges.Conversations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeviceQuery) loadConversations(ctx context.Context, query *ConversationQuery, nodes []*Device, init func(*Device), assign func(*Device, *Conversation)) error {
