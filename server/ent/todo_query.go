...
func (_q *TodoQuery) QueryConversation() *ConversationQuery {
...
func (_q *TodoQuery) WithConversation(opts ...func(*ConversationQuery)) *TodoQuery {
	query := (&ConversationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TodoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Todo, error) {
	...
	if query := _q.withConversation; query != nil {
		if err := _q.loadConversation(ctx, query, nodes, nil,
			func(n *Todo, e *Conversation) { n.Edges.Conversation = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TodoQuery) loadConversation(ctx context.Context, query *ConversationQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *Conversation)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Todo)
	for i := range nodes {
